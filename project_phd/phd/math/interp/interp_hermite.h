#ifndef MATH_INTERP_HERMITE_H
#define MATH_INTERP_HERMITE_H

#include "../math.h"
#include "interp.h"

/*
This file contains the interp_hermite class, which implements cubic
Hermite interpolation. Of all the different types of cubic splines,
this class implements those in which the first differential at the
nodes coincides with those provided by an average of Lagrange 1st order
interpolation or by Lagrange 2nd order interpolation.

Results provided by cubic interpolation are smooth in value and first
derivative, but not continuous in second derivated at the nodes 
*/
...
class MATH_API interp_hermite: public interp {
private:
	math::logic::INTERP_MODE _interp_mode;
	/**< interpolation mode */
	int _min_points;
	static const int _min_points_first;
	static const int _min_points_second;
...
public:
	interp_hermite(math::logic::INTERP_MODE);
	/**< constructor based on interpolation mode (only Hermite variants allowed) */
	interp_hermite(const interp_hermite&);
	/**< copy constructor */
	interp_hermite& operator=(const interp_hermite& op2);
