...
nav::filter_att_state03::filter_att_state03(const sens::suite& Osuite)
...
void nav::filter_att_state03::initialize() {
}
/* initialization */

void nav::filter_att_state03::update(const Eigen::Matrix<double,16,1>& xhat_aft_prev) {
...
Eigen::Matrix<double,16,1> nav::filter_att_state03::eval(const Eigen::Matrix<double,16,1>& xhat_aft_prev) const {
...
void nav::filter_att_state03::jacobian_state(Eigen::Matrix<double,16,16>& A, const Eigen::Matrix<double,16,1>& xhat_aft_prev) const {
...
Eigen::Matrix<double,16,16> nav::filter_att_state03::covariance_matrix() const {
    return _Qtilde;
}
...
nav::filter_att_obser03::filter_att_obser03(const sens::suite& Osuite, const env::geo& Ogeo)
...
nav::filter_att_obser03::~filter_att_obser03() {
...
void nav::filter_att_obser03::finalize_constructor(const double& Deltat_sec_nav) {
...
void nav::filter_att_obser03::initialize() {
...
void nav::filter_att_obser03::switch_to_gps_lost_mode() {
    _flag_gps = false;
...
void nav::filter_att_obser03::update(const Eigen::Matrix<double,16,1>& xhat_bef, const st::st_nav_out& Ost_nav_out_prev) {
...
Eigen::Matrix<double,9,1> nav::filter_att_obser03::eval(const Eigen::Matrix<double,16,1>& xhat_bef) const {
    ...
    res.segment<3>(3) = _w_nbb_rps.cross(_q_nb / _v_n_mps) + _q_nb / (_w_enn_rps.cross(_v_n_mps) - _gc_n_mps2_model + _a_cor_n_mps2) + _E_acc_mps2; // HERE is a change (for the worse) with respect to filter #02
...
void nav::filter_att_obser03::jacobian_state(Eigen::Matrix<double,9,16>& H, const Eigen::Matrix<double,16,1>& xhat) const {
    ...
    H.block<3,4>(3,0)  = _q_nb.jacobian_quat_backward_rotation(_w_enn_rps.cross(_v_n_mps) - _gc_n_mps2_model + _a_cor_n_mps2)
                         + ang::tools::skew3(_w_nbb_rps) * _q_nb.jacobian_quat_backward_rotation(_v_n_mps); // HERE is a change (for the worse) with respect to filter #02
    H.block<3,3>(3,4)  = ang::tools::right_skew3(_q_nb / _v_n_mps); // HERE is a change (for the worse) with respect to filter #02
...
void nav::filter_att_obser03::jacobian_noise(Eigen::Matrix<double,9,9>& M, const Eigen::Matrix<double,16,1>& xhat) const {
...
Eigen::Matrix<double,9,9> nav::filter_att_obser03::covariance_matrix() const {
    return _R;
}
/* returns covariance matrix */
...
nav::filter_att03::filter_att03(const sens::suite& Osuite, const env::geo& Ogeo)
: filter_att(Osuite), _Pekf_handler(nullptr), _Pstate(new nav::filter_att_state03(Osuite)), _Pobser(new nav::filter_att_obser03(Osuite, Ogeo)) {
}
/* constructor based on suite of sensors and Earth model */

nav::filter_att03::~filter_att03() {
...
void nav::filter_att03::finalize_constructor(const double& Deltat_sec_nav, const unsigned int& nel_nav) {
...
void nav::filter_att03::initialize(st::st_nav_out& Ost_nav_out_init, const st::st_sens_out& Ost_sens_out_init, const st::st_nav_in& Ost_nav_in_init,
    ...
    // I need to take into consideration the uncertainty of the initial quaternion when employing euler_id03 instead of euler_id00.
...
void nav::filter_att03::execute_step(st::st_nav_out& Ost_nav_out, const st::st_nav_out& Ost_nav_out_prev, const st::st_sens_out& Ost_sens_out, const st::st_nav_in& Ost_nav_in, const unsigned int& s) {
...
void nav::filter_att03::resize(const unsigned long& nel) {
    _Pekf_handler->resize(nel);
}
/* resize of all filter components to input size */

unsigned long nav::filter_att03::get_size() const {
    return _Pekf_handler->get_size();
}
/* returns size of all filter components */

void nav::filter_att03::switch_to_gps_lost_mode() {
