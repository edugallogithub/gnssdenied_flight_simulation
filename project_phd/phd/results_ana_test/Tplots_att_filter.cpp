...
nav::test::Tplots_att_filter::Tplots_att_filter(unsigned short& seed_init, unsigned short& seed_end)
: _seed_init(seed_init), _seed_end(seed_end) {
}
/* constructor based on counter */

void nav::test::Tplots_att_filter::run() {
    obtain_euler_att_filter_metrics         ("01_01_05_02_03_0100", _seed_init, _seed_end);
    obtain_vn_att_filter_metrics            ("01_01_05_02_03_0100", _seed_init, _seed_end);
    obtain_turn_location_att_filter_metrics ("01_01_05_02_03_0100", _seed_init, _seed_end);
    obtain_xhor_att_filter_metrics          ("01_01_05_02_03_0100", _seed_init, _seed_end);
}
...
void nav::test::Tplots_att_filter::obtain_euler_att_filter_metrics(const std::string& st_first_folder, unsigned short& seed_init, unsigned short& seed_end) {
    std::string st_folder      = st_first_folder;
    std::string st_folder_one  = "MAIN";
    std::string st_folder_one2 = "ATT_FILTER_03";
    std::string st_folder_one3 = "ATT_FILTER_04";
    ...
    std::string st_extra_folder("versus_att_filter");
    ...
    std::vector<std::vector<double>> VVxi_deg(nel),ZZxi_deg(nel), YYxi_deg(nel);
    std::vector<double> Vangle_deg_all(nel_all), Zangle_deg_all(nel_all), Yangle_deg_all(nel_all);
    std::vector<double> Vxi_deg_all(nel_all), Zxi_deg_all(nel_all), Yxi_deg_all(nel_all);
    ...
    for (unsigned short seed = seed_init; seed <= seed_end; ++seed) {
        ...
        VVxi_deg[seed-seed_init].resize(nel_sec);
        ...
        ZZxi_deg[seed-seed_init].resize(nel_sec);
        ...
        do {
            ...
            if (a01 > (-1e-8)) {
                ...
                Zxi_deg_all[index_all] = a10;
                index_all++;
                if (fabs(dec_part) < 1e-8) {
                    Vt_sec[index_sec] = a01;
                    ZZangle_deg[seed-seed_init][index_sec] = a11;
                    ZZxi_deg[seed-seed_init][index_sec] = a10;
                    index_sec++;
                }
            }
        } while (index_sec != nel_sec);
        ...
        YYxi_deg[seed-seed_init].resize(nel_sec);
        ...
        do {
            ...
            if (a01 > (-1e-8)) {
                ...
                Yxi_deg_all[index_all] = a10;
                index_all++;
                if (fabs(dec_part) < 1e-8) {
                    Vt_sec[index_sec] = a01;
                    YYangle_deg[seed-seed_init][index_sec] = a11;
                    YYxi_deg[seed-seed_init][index_sec] = a10;
                    index_sec++;
                }
            }
        } while (index_sec != nel_sec);
    ...
    std::cout << setw(10) << " " << setw(21) << "baseline" << setw(21) << "Zero Body Differentials" << setw(21) << "Zero NED Differentials" << std::endl;
    std::cout << setw(10) << " "
    ...
    std::vector<std::vector<double>> WWxi_deg(nel_sec), WWxi_deg_2(nel_sec), WWxi_deg_3(nel_sec);
    ...
    std::vector<double> Wmean_xi_deg(nel_sec), Wstd_xi_deg(nel_sec), Wmean_xi_deg_2(nel_sec), Wstd_xi_deg_2(nel_sec), Wmean_xi_deg_3(nel_sec), Wstd_xi_deg_3(nel_sec);
    for (unsigned long i = 0; i != nel_sec; ++i) {
        WWangle_deg[i].resize(nel);
        WWangle_deg_2[i].resize(nel);
        WWangle_deg_3[i].resize(nel);
        WWxi_deg[i].resize(nel);
        WWxi_deg_2[i].resize(nel);
        WWxi_deg_3[i].resize(nel);
        for (unsigned short seed = seed_init; seed <= seed_end; ++seed) {
            ...
            WWxi_deg[i][seed-seed_init]      = VVxi_deg[seed-seed_init][i];
            WWxi_deg_2[i][seed-seed_init]    = ZZxi_deg[seed-seed_init][i];
            WWxi_deg_3[i][seed-seed_init]    = YYxi_deg[seed-seed_init][i];
        }
        ...
        Wmean_xi_deg[i]      = math::mean(WWxi_deg[i]);
        Wstd_xi_deg[i]       = math::std(WWxi_deg[i], Wmean_xi_deg[i]);
        Wmean_xi_deg_2[i]    = math::mean(WWxi_deg_2[i]);
        Wstd_xi_deg_2[i]     = math::std(WWxi_deg_2[i], Wmean_xi_deg_2[i]);
        Wmean_xi_deg_3[i]    = math::mean(WWxi_deg_3[i]);
        Wstd_xi_deg_3[i]     = math::std(WWxi_deg_3[i], Wmean_xi_deg_3[i]);
    ...
    std::string st_file_out = "versus_att_filter_euler_deg.txt";
    ...
    for (unsigned long i = 0; i != nel_sec; ++i) {
        Oout << fixed      << setw(8)  << setprecision(1) << showpos << Vt_sec[i]
             ...
             << scientific << setw(12) << setprecision(3) << showpos << Wmean_angle_deg_3[i] - Wstd_angle_deg_3[i]
             << scientific << setw(12) << setprecision(3) << showpos << Wmean_xi_deg[i]
             << scientific << setw(12) << setprecision(3) << showpos << Wmean_xi_deg[i] + Wstd_xi_deg[i]
             << scientific << setw(12) << setprecision(3) << showpos << Wmean_xi_deg[i] - Wstd_xi_deg[i];
        Oout << endl;
    }
    ...
    std::string st_file_out_tex = "versus_att_filter_euler_table.tex";
    ...
    Oout_tex << "\\textbf{Benchmark} & \\multicolumn{9}{c}{\\textbf{Attitude Filter Configuration}} \\\\" << std::endl;
    Oout_tex << "\\nm{\\| \\phiNBest - \\phiNB \\| \\ \\lrsb{deg}} & \\multicolumn{3}{c}{\\textbf{Baseline}} & \\multicolumn{3}{c}{\\textbf{Zero \\nm{\\FB} Diffs.}} & \\multicolumn{3}{c}{\\textbf{Zero \\nm{\\FN} Diffs.}} \\\\" << std::endl;
    Oout_tex << "\\hline" << std::endl;
 ...
 * - "versus_att_filter_euler_deg.txt" --> to plot euler mean and std variation with time
 * - "versus_att_filter_euler_table.tex" --> script to directly generate table in Latex */


void nav::test::Tplots_att_filter::obtain_vn_att_filter_metrics(const std::string& st_first_folder, unsigned short& seed_init, unsigned short& seed_end) {
    std::string st_folder          = st_first_folder;
    std::string st_folder_one      = "MAIN";
    std::string st_folder_one_gyr2 = "ATT_FILTER_03";
    std::string st_folder_one_gyr3 = "ATT_FILTER_04";
    ...
    std::string st_seeds, st_file_gps_complete, st_first_line, st_line, st_file_gps_complete_gyr2, st_file_gps_complete_gyr3;
    std::string st_file_pos_complete, st_file_pos_complete_gyr2, st_file_pos_complete_gyr3;
    unsigned long nel     = (unsigned long)(seed_end) - (unsigned long)(seed_init) + 1;
    unsigned long nel_all = 380001;
    unsigned long nel_sec = 381;
    unsigned long index_all, index_sec;
    ...
    for (unsigned short seed = seed_init; seed <= seed_end; ++seed) {
        ...
        st_file_gps_complete_gyr2 = (path_outputs_read / st_folder_main / st_folder_one_gyr2 / st_folder / st_file_gps).string();
        st_file_gps_complete_gyr3 = (path_outputs_read / st_folder_main / st_folder_one_gyr3 / st_folder / st_file_gps).string();
        st_file_pos_complete      = (path_outputs_read / st_folder_main / st_folder_one      / st_folder / st_file_pos).string();
        st_file_pos_complete_gyr2 = (path_outputs_read / st_folder_main / st_folder_one_gyr2 / st_folder / st_file_pos).string();
        st_file_pos_complete_gyr3 = (path_outputs_read / st_folder_main / st_folder_one_gyr3 / st_folder / st_file_pos).string();
        ...
        std::ifstream Ostream_gps_gyr2;
        Ostream_gps_gyr2.open(st_file_gps_complete_gyr2);
        std::getline(Ostream_gps_gyr2, st_first_line);
        ...
        while (std::getline(Ostream_gps_gyr2, st_line)) {
            st_t_sec          = st_line.substr(  0, 10);
            st_vni_mps_est    = st_line.substr( 10, 14);
            st_vnii_mps_est   = st_line.substr( 24, 14);
            st_vni_mps_truth  = st_line.substr( 94, 14);
        ...
        Ostream_gps_gyr2.close();

        std::ifstream Ostream_pos_gyr2;
        Ostream_pos_gyr2.open(st_file_pos_complete_gyr2);
        std::getline(Ostream_pos_gyr2, st_first_line);
        std::getline(Ostream_pos_gyr2, st_line); // line repeated at 100 [sec] from previous file
        while (std::getline(Ostream_pos_gyr2, st_line)) {
            st_t_sec          = st_line.substr(  0, 10);
            st_vni_mps_est    = st_line.substr( 10, 14);
            st_vnii_mps_est   = st_line.substr( 24, 14);
            st_vni_mps_truth  = st_line.substr( 52, 14);
        ...
        Ostream_pos_gyr2.close();


        std::ifstream Ostream_gps_gyr3;
        Ostream_gps_gyr3.open(st_file_gps_complete_gyr3);
        std::getline(Ostream_gps_gyr3, st_first_line);
        ...
        while (std::getline(Ostream_gps_gyr3, st_line)) {
            st_t_sec          = st_line.substr(  0, 10);
            st_vni_mps_est    = st_line.substr( 10, 14);
            st_vnii_mps_est   = st_line.substr( 24, 14);
            st_vni_mps_truth  = st_line.substr( 94, 14);
        ...
        Ostream_gps_gyr3.close();

        std::ifstream Ostream_pos_gyr3;
        Ostream_pos_gyr3.open(st_file_pos_complete_gyr3);
        std::getline(Ostream_pos_gyr3, st_first_line);
        std::getline(Ostream_pos_gyr3, st_line); // line repeated at 100 [sec] from previous file
        while (std::getline(Ostream_pos_gyr3, st_line)) {
            st_t_sec          = st_line.substr(  0, 10);
            st_vni_mps_est    = st_line.substr( 10, 14);
            st_vnii_mps_est   = st_line.substr( 24, 14);
            st_vni_mps_truth  = st_line.substr( 52, 14);
    ...
    std::cout << setw(10) << " " << setw(24) << "baseline" << setw(24) << "Zero Body Differentials" << setw(24) << "Zero NED Differentials" << std::endl;
    ...
    std::string st_file_out = "versus_att_filter_vned_mps.txt";
    std::string st_file_output = (path_outputs_write / st_folder_main / st_extra_folder / st_file_out).string();
    ...
    std::string st_file_out_tex = "versus_att_filter_vned_table.tex";
    std::string st_file_output_tex = (path_outputs_write / st_folder_main / st_extra_folder / st_file_out_tex).string();
    ...
    Oout_tex << "\\hline" << std::endl;
    Oout_tex << "\\textbf{Benchmark} & \\multicolumn{8}{c}{\\textbf{Attitude Filter Configuration}} \\\\" << std::endl;
    Oout_tex << " & \\multicolumn{2}{c}{\\textbf{Baseline}} & & \\multicolumn{2}{c}{\\textbf{Zero \\nm{\\FB} Diffs.}} & & \\multicolumn{2}{c}{\\textbf{Zero \\nm{\\FN} Diffs.}} \\\\" << std::endl;
 ...
 * different attitude filter implementations.
 * It shows results on console and also generates the following files (for thesis, not Matlab):
 * - "versus_att_filter_vned_mps.txt" --> to plot ground speed final error
 * - "versus_att_filter_vned_table.tex" --> script to directly generate table in Latex */

void nav::test::Tplots_att_filter::obtain_turn_location_att_filter_metrics(const std::string& st_first_folder, unsigned short& seed_init, unsigned short& seed_end) {
    std::string st_folder      = st_first_folder;
    std::string st_folder_one  = "MAIN";
    ...
    unsigned long nel_sec = 501; // one per second until 500 [sec] only

    std::vector<double> Vt_sec(nel_sec);
    std::vector<int> Vid(nel_sec,0);

    std::string st_seeds, st_file_complete, st_line, st_first_line, st_t_sec, st_id;
    double int_part, dec_part, t_sec;
    int id;
    ...
    //for (unsigned int i = 0; i != nel_sec; ++i) {
    //    std::cout << Vt_sec[i] << "   " << Vid[i] << std::endl;
    //}

    // convert into vector of doubles that has number of ocurrences divided by 10
    std::vector<double> Vidid(nel_sec,0.);
    for (unsigned int i = 0; i != nel_sec; ++i) {
        Vidid[i] = Vid[i] / 10.0;
    }
    ...
    std::string st_file_out = "versus_att_filter_turn_location.txt";
    std::string st_file_output = (path_outputs_write / st_folder_main / st_extra_folder / st_file_out).string();
    ...
    for (unsigned long i = 0; i != nel_sec; ++i) {
        Oout << fixed << setw(8)  << setprecision(1) << showpos << Vt_sec[i]
             << fixed << setw(8) << setprecision(2)  << showpos << Vidid[i];
        Oout << endl;
    }
    Oout.close();
}
...
void nav::test::Tplots_att_filter::obtain_xhor_att_filter_metrics(const std::string& st_first_folder, unsigned short& seed_init, unsigned short& seed_end) {
    std::string st_folder          = st_first_folder;
    std::string st_folder_one      = "MAIN";
    std::string st_folder_one_gyr2 = "ATT_FILTER_03";
    std::string st_folder_one_gyr3 = "ATT_FILTER_04";
    ...
    for (unsigned short seed = seed_init; seed <= seed_end; ++seed) {
        ...
        st_folder.replace(3,2,st_seeds);
        st_file_complete   = (path_outputs_read / st_folder_main / st_folder_one      / st_folder / st_file).string();
        st_file_complete_2 = (path_outputs_read / st_folder_main / st_folder_one_gyr2 / st_folder / st_file).string();
        st_file_complete_3 = (path_outputs_read / st_folder_main / st_folder_one_gyr3 / st_folder / st_file).string();
    ...
    std::cout << setw(10) << " " << setw(28) << "baseline" << setw(28) << "Zero Body Differentials" << "Zero NED Differentials" << std::endl;
    std::cout << setw(10) << " "
    ...
    std::string st_file_out = "versus_att_filter_pos_hor_m_pc.txt";
    std::string st_file_output = (path_outputs_write / st_folder_main / st_extra_folder / st_file_out).string();
    ...
    std::string st_file_out_tex = "versus_att_filter_pos_hor_table.tex";
    std::string st_file_output_tex = (path_outputs_write / st_folder_main / st_extra_folder / st_file_out_tex).string();
    ...
    Oout_tex << "\\hline" << std::endl;
    Oout_tex << "\\textbf{Benchmark} & \\multicolumn{9}{c}{\\textbf{Attitude Filter Configuration}} \\\\" << std::endl;
    Oout_tex << " & \\multicolumn{3}{c}{\\textbf{Baseline}} & & \\multicolumn{2}{c}{\\textbf{Zero \\nm{\\FB} Diffs.}} & & \\multicolumn{2}{c}{\\textbf{Zero \\nm{\\FN} Diffs.}} \\\\" << std::endl;
 ...
 * different attitude filter implementations.
 * It shows results on console and also generates the following files (for thesis, not Matlab):
 * - "versus_att_filter_pos_hor_m_pc.txt" --> to plot horizontal error mean and std variation with time
 * - "versus_att_filter_pos_h.tex" --> script to directly generate table in Latex */
